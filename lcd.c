...
void HAL_LCD_SpiInit(void)
{
    ...
    EUSCI_B0->CTLW0 = EUSCI_B_CTLW0_CKPH |        // phase of one
                      EUSCI_B_CTLW0_MSB |         // MSB first
                      EUSCI_B_CTLW0_MST |         // Set as SPI leader
...
void HAL_LCD_writeCommand(uint8_t command)
{
    ...
    while(EUSCI_B0->STATW & UCBUSY) {   // UCBUSY high when tx/rx
    }
    // Transmit data
    EUSCI_B0->TXBUF = command; // command is an 8 bit parameter
    // Busy wait while the data is being transmitted. Check the STATW register and see if it is busy 
    while(EUSCI_B0->STATW & UCBUSY) {   // UCBUSY high when tx/rx
    }
...
void HAL_LCD_writeData(uint8_t data)
{
    ...
    // Transmit data
    EUSCI_B0->TXBUF = data; // data is 8bit
    // Busy wait while the data is being transmitted. Check the STATW register and see if it is busy 
    while(EUSCI_B0->STATW & UCBUSY) {   // UCBUSY high when tx/rx
    }
}
...
/*******************************************************************************
* Function Name: lcd_draw_board
********************************************************************************
* Summary: Prints a board centered
* Returns:
*  Nothing
*******************************************************************************/
void lcd_draw_board(uint16_t image_color) {

    //buttons
    lcd_draw_image(112, 20, button1WidthPixels, button1HeightPixels, button1Bitmaps,
                   B1_COLOR, LCD_COLOR_BLACK);

    lcd_draw_image(110, 110, button2imageWidthPixels, button2imageHeightPixels, button2imageBitmaps,
                   B2_COLOR, LCD_COLOR_BLACK);

    //arrows
    lcd_draw_image(66, 14, arrow_upWidthPixels, arrow_upHeightPixels, arrow_upBitmaps,
                   ACCELEROMETER_COLOR_UP, LCD_COLOR_BLACK);

    lcd_draw_image(112, 66, arrow_rightWidthPixels, arrow_rightHeightPixels, arrow_rightBitmaps,
                   ACCELEROMETER_COLOR_RIGHT, LCD_COLOR_BLACK);

    lcd_draw_image(66, 118, arrow_downWidthPixels, arrow_downHeightPixels, arrow_downBitmaps,
                   ACCELEROMETER_COLOR_DOWN, LCD_COLOR_BLACK);

    lcd_draw_image(16, 66, arrow_leftWidthPixels, arrow_leftHeightPixels, arrow_leftBitmaps,
                   ACCELEROMETER_COLOR_LEFT, LCD_COLOR_BLACK);

    //lightbulb
    lcd_draw_image(17, 24, lightbulbWidthPixels, lightbulbHeightPixels, lightbulbBitmaps,
                   AMB_LIGHT_COLOR, LCD_COLOR_BLACK);
}

void lcd_draw_bop_it(uint16_t image_color, uint8_t x, uint8_t y) {
    lcd_draw_image(x, y, bopitWidthPixels, bopitHeightPixels, bopitBitmaps,
                       image_color, LCD_COLOR_BLACK);
}
